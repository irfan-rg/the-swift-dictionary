  ...
  if (success) {
    return (
      <div className="flex items-center justify-center min-h-screen py-16 px-4 max-md:items-start max-md:px-0 max-md:py-0 max-md:pt-6">
        <div className="w-full max-w-md max-md:max-w-none rounded-sm max-md:rounded-none border border-[var(--border)] max-md:border-0 max-md:border-b bg-[var(--surface-raised)] max-md:bg-transparent p-8 max-md:px-6 max-md:py-8 text-center">
          <div className="w-14 h-14 rounded-sm border border-[var(--border)] grid place-items-center mx-auto mb-4">
            <Mail className="w-6 h-6 text-[var(--accent)]" />
          </div>
          <h2 className="font-display text-2xl font-medium text-[var(--foreground)] mb-2">
            Check your email
          </h2>
          <p className="font-body text-sm text-[var(--foreground-muted)] mb-6">
            We sent a confirmation link to <strong className="text-[var(--foreground)]">{email}</strong>. Click the link to activate your account.
          </p>
          <Link
            href="/auth/login"
      ...
      <div className="w-full max-w-md max-md:max-w-none rounded-sm max-md:rounded-none border border-[var(--border)] max-md:border-0 max-md:border-b bg-[var(--surface-raised)] max-md:bg-transparent p-8 max-md:px-6 max-md:py-8">
        <div className="text-center mb-8">
          ...
          <p className="font-body text-sm text-[var(--foreground-muted)]">
            Join The Swift Dictionary to save favorites
          </p>
        </div>

        <form onSubmit={handleSignup} className="space-y-4">
          <div>
            <label htmlFor="name" className="block font-body text-[10px] tracking-widest uppercase text-[var(--foreground-muted)] mb-2">
              Display Name
            </label>
            <div className="relative">
              <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--foreground-muted)] opacity-50" />
              <input
                id="name"
                type="text"
                value={displayName}
